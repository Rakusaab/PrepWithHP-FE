...
export default function DashboardPage() {
  ...
  if (!userId || perfLoading || weakLoading || testLoading) {
    return (
      <DashboardLayout>
  ...
  // Ensure weakAreas is an array before mapping
  const weakAreasArray = Array.isArray(weakAreas) ? weakAreas : [];
  const mappedWeakAreas = weakAreasArray.map((w: any) => ({
    id: w.id?.toString() ?? '',
    subject: w.subject || w.name || '',
    topic: w.topic || '',
    examCategory: w.exam_category || '',
    averageScore: w.average_score ?? 0,
    attemptsCount: w.attempts_count ?? 0,
    lastAttempted: w.last_attempted ?? '',
    difficulty: w.difficulty ?? 'medium',
    improvementSuggestions: w.improvement_suggestions ?? [],
      ...
      <div className="p-6 space-y-6">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="improvement" className="space-y-6">
            <WeakAreasAnalysis weakAreas={mappedWeakAreas} />
            <PersonalizedRecommendations weakAreas={mappedWeakAreas} />
          </TabsContent>
